...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - V28_ENGINE - %(levelname)s - %(message)s')
...
def build_v28_payload():
    logger.info("⚡ INITIATING V28 ULTIMATE MASTERPIECE ENGINE ⚡")
    ...
    for i, phase_text in enumerate(phases):
        ...
        audio_path = os.path.join(public_dir, f"v28_audio_{i}.mp3")
    ...
    json_path = os.path.join(public_dir, "v28_script.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, indent=2)
        
    logger.info("Triggering Remotion Render (V28 - Ultimate Masterpiece)...")
    studio_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "remotion-studio"))
    out_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "FINAL_V28_MASTERPIECE.mp4"))
    ...
    logger.info(f"✅ SUCCESSFULLY RENDERED V28 REEL: {out_file}")
